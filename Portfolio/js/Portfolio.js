...
function setupPage() {
  if (window.location.href.includes(`/Portfolio?project=`)) {
...
function createProjects() {
  ...
  for (let project of projects) {
    ...
    let image = project["image"].split(".")[0] + "512." + project["image"].split(".")[1];
    ...
    let elementForAdd = `<div class="flex-item" id="portfolio-${title.replace(/[^a-z0-9]/gim, "-")}" style="background-image: url(projects/${image})"><div><div class="bottomText">${title}</div></div></div>`;
  ...
  //   let title2 = project2["title"];
  //   let href2 = `?project=portfolio-${title2.replace(/[^a-z0-9]/gim, "-")}`;
  //   let genFunc = function name() {};
  //   genFunc = onclickFunction(`${title2.replace(/[^a-z0-9]/gim, "")}`, genFunc, href2);
  //   // console.log("generated onclick function for element:", href2);
  //   // console.log(genFunc);
  //   document.getElementById(`portfolio-${title2.replace(/[^a-z0-9]/gim, "-")}`).onclick = genFunc;
  // }
  //call createLightbox with the classname of the element clicked if a child of #bodyParent > .flex-container is clicked
  for (let projectelement of document.getElementsByClassName("flex-item")) {
    projectelement.onclick = function () {
      console.log("clicked project");
      createLightbox(projectelement.id);
    };
  }
}

//prehaps the longest function here, this sets up the lightbox
function createLightbox(source_id) {
  ...
  let featureImg = element.style["background-image"].replace("512", "");

  //same as previous, steals the title text from the preview you clicked on
  let bottomText = element.children[0].children[0].textContent;
  ...
  for (let line of projects) {
    ...
    if (line["title"] == bottomText) {
      ...
      if (line["slideshow"] != undefined) {
        ...
        let dotParent = document.getElementById("slideDots");
        for (var i = 1; i < line["slideshow"].length + 1; i++) {
          let renderType = "O";
          let renderTypeLong = "other";
          try {
        ...
        for (let slideImg of line["slideshow"]) {
          ...
          let index = line["slideshow"].indexOf(slideImg);
          ...
          if (slideImg.includes(".png") || slideImg.includes(".jpg") || slideImg.includes(".webp") || slideImg.includes(".gif")) {
            ...
            let fileExtention = slideImg.split(".")[slideImg.split(".").length - 1];
  ...
  for (let line of projects) {
    ...
    if (line["title"] == bottomText) {
      ...
      if (line["sketchfab"] != undefined) {
        let dotParent = document.getElementById("sketchDots");
        ...
        var sketchHTML = ``;
        for (let sketch of line["sketchfab"]) {
...
function currentSlide(n) {
  ...
  if (document.getElementsByClassName("dot")[n - 1].innerHTML.includes("V")) {
    n = 88;
  }
